...
MVP (v0.1) ogranicza się **wyłącznie do small.pl/Devil** i tylko do wybranego wycinka P0 z [PRD §6](./PRD.md#6-ficzery--z-priorytetami). v0.2 dorzuca **jednego, świadomie wybranego** drugiego providera (kandydaci: cPanel, DirectAdmin — wybór po review notatek badawczych) plus live log stream, GHA monitor i pełny Command Palette. v0.3+ — multi-provider, in-app updater, scaffolding kolejnych stacków, eksport/import konfiguracji. Wersja **v1.0** wymaga 3 mies. stabilności + co najmniej 1 community-provided provider zewnętrzny.
...
2. Co najmniej **1 community-provided provider** zewnętrzny (kryterium K6 z [PRD §8](./PRD.md#8-kryteria-sukcesu--mierzalne)).
...
- **F3** Wizard nowego projektu (5 kroków, **smart skip DB** — [poprawka 6.10](../CHANGES.md#1-poprawki-merytoryczne-z-tabeli-%C2%A76-briefu)).
...
8–12 tygodni jednoosobowej pracy maintainera, lub 4–6 tygodni przy 2-osobowym zespole. Ścieżka krytyczna: SSH layer + Provider Pattern + wizard + integracja GH = ~60 % czasu. Reszta to UI polerka i testy.
