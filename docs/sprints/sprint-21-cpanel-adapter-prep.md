...
> **Daty:** 2026-06-08 → 2026-06-22 (2 tygodnie, zrealizowano 2026-05-25 w jednej sesji autonomicznej) · **Cel:** Pierwsza warstwa cPanel adaptera (UAPI client + read-only ops + `webox doctor cpanel`) i jednoczesne przygotowanie public launch — repo polish, asciinema, cPanel test account.
>
> **Status:** ✅ Closed 2026-05-25 (6 of 8 tasks done; TASK-21.7 deferred to Sprint 22 on maintainer ops, TASK-21.8 = this outcome + [retro](../retros/2026-05-25-sprint-21.md)). **Properties:** code + ops mix · **Path selected:** **A** (full parallel — code + ops tracks both active).
...
- ✅ Done:
  ...
  - **TASK-21.3 (2026-05-25)** — `webox doctor cpanel --host=H --user=U [--token=T] [--api-port=N] [--ssh-port=N] [--timeout=DUR] [--no-ssh] [--no-uapi] [--json]` exercises all four read-only modules in a single call. Pure validation + rollup logic in [`cmd/webox/cpanel.go`](../../cmd/webox/cpanel.go); native-SSH runner in [`cmd/webox/cpanel_runner.go`](../../cmd/webox/cpanel_runner.go) (same `BatchMode=yes` profile as TASK-21.4). Section status taxonomy: `OK` / `DISABLED` (DISABLED counts as OK because it's not a Webox failure) / `AUTH_FAILED` / `UNREACHABLE` / `FAILED`. Rollup verdict: `OK` (all OK or DISABLED) / `DEGRADED` (mixed; exits 0) / `BLOCKED` (all-failed; exits 1). 23 unit tests cover the validation matrix, rollup 7-case table, builder wiring (HTTPS-only / SSH-only / composite), happy / DEGRADED / BLOCKED outcomes, text + JSON output stability, CLI flag parsing including the `provider new cpanel <X>` vs `doctor cpanel` ambiguity (resolved by making `simpleFlagHandled` and `applyContextualToken` state-aware so `provider new cpanel myadapter` still parses correctly). Bug discovered + fixed during smoke test: `transport.sleepWithCtx` now wraps `ctx.Err()` with `ErrTransportUnavailable` so context-cancelled retries surface as `UNREACHABLE` not generic `FAILED`. End-to-end smoke (`./bin/webox doctor cpanel --host=192.0.2.1 --user=test --token=FAKE --no-ssh --timeout=2s`) returns 4× `UNREACHABLE` + `BLOCKED` verdict + exit 1 — confirms HTTPS path wired and error mapping correct.
  - **TASK-21.1 + TASK-21.2 (2026-05-25)** — cPanel UAPI read-only client + SSH fallback land together as the standalone, importable transport layer the future `providers/cpanel` adapter will compose. HTTPS path (`providers/cpanel/uapi/transport.go`, `client.go`): typed `Client.{ListDomains, ListPassengerApps, ListMysqlDatabases, ListSSLKeys}` over `https://host:2083/execute/<Module>/<Function>`, HTTPS-only constructor, basic-auth via `Authorization: cpanel <user>:<token>`, exponential backoff on 429 / 5xx (500 ms × 2ⁿ, capped at 3 retries + injectable backoff for fast tests), 4 MiB body cap, every error mapped to a typed sentinel (10 sentinels in `errors.go` keep CLI callers from string-matching). SSH path (`ssh.go`, `sshpool.go`): `SSHFallback` shells out to `uapi --user=<user> --output=jsonpretty <Module> <function>`; the `SSHRunner` seam keeps tests dependency-free while the production `SSHPoolRunner` wraps `ssh.Exec` and distinguishes non-zero-exit (exitCode + stderr) from transport failure (`ErrTransportUnavailable`). `shellQuote` defends future user-typed arg maps. Composite layer (`composite.go`): closed `Reader` interface plus generics-driven `Composite{Primary, Secondary}` that prefers HTTPS, fails over to SSH on `errors.Is(err, ErrTransportUnavailable)`, surfaces auth / rate-limit / malformed / module-denied verbatim (they'd hit the same wall on the other side). Generic shape-tolerant decoders (`decoders.go`) accept modern object-wrapper + top-level array + legacy map-keyed shapes (cPanel < 88), promoting the legacy key into the row's `Name` and stable-sorting for snapshot determinism. `MutatingClient` interface returns `ErrSprintScopeNotMutable` until Sprint 22 — type-system-enforced "no destructive ops in v0.2-rc" guardrail. 7 golden fixtures (`testdata/`) cover happy + edge paths (legacy shape, module disabled, generic API failure); they're research-derived from public api.docs.cpanel.net until TASK-21.7 onboards a live test account. Coverage: **77.9%** across **24 unit tests** — constructor validation, 4 happy paths × 2 transports, legacy map shape, transient + terminal error matrix, retry budget assertion, context cancellation, 10-case shell-quote table (incl. `$(rm -rf /)` / `` `whoami` `` / `O'Brien` injection attempts), composite fall-over / fall-through, transport-unavailable mapping on RFC 5737 unreachable IP.
- ⏭️ Carry-over:
  - **TASK-21.7 → Sprint 22** — Live cPanel test account onboarding is blocked on maintainer-side procurement (vendor purchase + H88 partnership outreach). The 7 research-derived fixtures in `providers/cpanel/uapi/testdata/` stay in place until the live account lands; once it does, replace them one-for-one and graduate the `cpanel-generic` preset from `research` to `verified`.
  - Sprint 22 should treat the test-account onboarding as P0 before tagging `v0.2.0-rc1` so the cPanel adapter ships with live-validated fixtures.
- 📌 Decyzje:
  - No new ADRs in Sprint 21. The architecture choices documented in [ADR-0008](../adr/0008-preset-registry.md) and [ADR-0009](../adr/0009-sprint-renumber-post-19.md) cover the preset registry + sprint renumbering basis; the read-only-only UAPI scope is documented inline in `providers/cpanel/doc.go` + `providers/cpanel/uapi/doc.go`. Sprint 22 will need a new ADR for the mutating-ops architecture (env-var guard + WHM API token vs cPanel API token, etc.).
- 🧠 Surprises:
  - **cPanel legacy map shape still in production.** Some cPanel < 88 installs return PassengerApps + Mysql.list_databases as map-keyed objects, not arrays. The decoder handles both transparently; without the heads-up from public api.docs.cpanel.net commit history this would have hit us during fixture capture in Sprint 22.
  - **Context-cancelled retries went to `FAILED` not `UNREACHABLE`.** Discovered via end-to-end smoke against RFC 5737 unreachable IP; fix wraps `ctx.Err()` with `ErrTransportUnavailable` in `transport.sleepWithCtx`. Unit tests using zero-backoff never exercised the cancel-during-retry path.
  - **`provider new cpanel <name>` vs `doctor cpanel` parser collision.** The first fix (reordering `parseArgs`) broke the `["provider", "new", "cpanel"]` case. The right shape is state-aware token whitelisting in `simpleFlagHandled` via `isPendingProviderNameSlot(parsed)`. Documented in the retro.
- 📊 Metrics:
  - **`providers/cpanel/uapi/` package:** 11 source files, 24 unit tests, 7 golden fixtures, **77.9 %** coverage. Lint 0 issues.
  - **`webox doctor cpanel`:** 23 unit tests, lint 0 issues, end-to-end smoke (RFC 5737 unreachable IP) returns 4× `UNREACHABLE` + `BLOCKED` verdict + exit 1.
  - **Project-wide:** `make ci` green; total coverage **79.3 %** (≥ 70 % bar).
  - **asciinema cast (TASK-21.6):** 13.6 KB cast + 98 KB GIF, recorded deterministically via `expect` at `120×35`.
  - **Sprint velocity:** 6 of 8 planned tasks closed in a single autonomous session. TASK-21.7 deferred to Sprint 22 on maintainer-ops dependency.
