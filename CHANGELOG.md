...
- **Sprint 22 TASK-22.7 — Sprint 22 retro (2026-05-26).** [`docs/retros/2026-05-26-sprint-22.md`](./docs/retros/2026-05-26-sprint-22.md) captures the session: 5 of 8 tasks closed (TASK-22.1, 22.2, 22.3, 22.4, 22.5); TASK-22.0 deferred because operator opted to defer credential rotation; TASK-22.6 (`v0.2.0-rc1` release tag) blocked on TASK-22.0 per the sprint plan's gating criteria. What worked: `Mutator` interface mirrored `Reader` pattern, `classifyMutationError` collapsed 16 idempotency call-sites to one, retry-class refactor saved ~56s of test wall-clock, env-var guard short-circuits before construction validation, `PrepareWorkflowData` keeps the wizard provider-agnostic, E2E test uses real HTTPS transport. Friction: cleartext password leak forced 10min of security recovery before any code landed, `assets/workflows/cpanel-uapi.yml` per-provider plan didn't fit the post-Sprint-14 per-stack layout (resolved by `providers/cpanel/workflow.go` helper), `t.Parallel`+`t.Setenv` incompatibility, gocyclo flagged `parseProperties`, pre-commit hook flagged test fixtures as secrets (false positive). Carry-over to Sprint 23: TASK-22.0 (live fixtures, gated on rotation), TASK-22.6 (rc1 tag, gated on TASK-22.0), TUI provider-picker, mutating_test retry coverage, `{user}` vs `{{user}}` template syntax unification, `webox provider new cpanel` generator wiring for the cpanel-uapi preset.

- **Sprint 22 TASK-22.4 — cPanel GHA workflow integration (2026-05-26).** Adds [`providers/cpanel/workflow.go`](./providers/cpanel/workflow.go) exposing `WorkflowRestartCommand(appPath)`, `WorkflowRsyncExcludes()`, and `(*Provider).PrepareWorkflowData(domain, extraExcludes)` so the wizard hands the existing per-stack workflow templates ([`assets/workflows/{static,node-express,vite-react}/deploy.yml`](./assets/workflows/)) the cPanel-flavoured restart command (Passenger panel-agnostic `touch <app>/tmp/restart.txt`) plus cpanel-friendly rsync excludes (`tmp/`, `.env`, `node_modules/`, `public_html/`, `.htaccess`, `cgi-bin/`). The restart command deliberately avoids the UAPI `PassengerApps::restart_application` endpoint so the GHA deploy never carries an API token — SSH + filesystem access is enough. Defence-in-depth: single quotes are stripped from `appPath` before substitution so a malicious `app_root_template` override cannot weaponise the workflow renderer, and the wizard's existing `ValidateWorkflowField` rejects `${{`, backticks, `$(`, and newlines at the field level. 9 new unit tests in [`providers/cpanel/workflow_test.go`](./providers/cpanel/workflow_test.go) (touch-restart formatter, custom layouts, single-quote stripping, whitespace trim, panel-critical excludes assertion, fresh-slice guarantee, happy-path / invalid-domain / custom-template integration); 2 new integration tests in [`assets/workflows_test.go`](./assets/workflows_test.go) (cpanel-shaped node-express render + shell-injection rejection). The sprint plan's call for a separate `assets/workflows/cpanel-uapi.yml` was reconciled against the post-Sprint-14 per-stack template layout: cPanel inherits the existing `node-express/deploy.yml` template and supplies its own `RestartCommand` + excludes through `WorkflowData`, which is more robust than duplicating the template.
- **Sprint 22 TASK-22.5 — cPanel adapter E2E test suite (2026-05-26).** New [`internal/e2e/cpanel_test.go`](./internal/e2e/cpanel_test.go) drives the cpanel adapter through `wizard.Execute` against a real `httptest.NewTLSServer` running the production `uapi.NewClient` + `uapi.NewHTTPSMutator` — no in-memory fakes. Four scenarios: (1) **Happy path** — wizard provisions AddAddonDomain → CreatePassengerApp → StartAutoSSL → CreateMysqlDatabase → CreateMysqlUser → SetMysqlPrivileges in <15ms, all three cleanup steps pushed onto the LIFO stack; (2) **SSL failure rollback** — server returns 500 on `SSL/start_autossl_check`, wizard halts at SetupSSL, the subsequent `Stack.Rollback` issues `PassengerApps/delete_application` BEFORE `DomainInfo/del_domain` (verifying the adapter's mandatory ordering — the panel refuses to drop a domain with a live Passenger app); (3) **Domain collision pre-check** — `wizard.CheckSubdomainAvailable` returns `providers.ErrSubdomainExists` after a single `PassengerApps/list_applications` call, with zero mutating endpoints touched (defends against the wizard accidentally leaking a partial mutation past the collision guard); (4) **Token expired (401)** — the HTTPS transport classifies 401 as terminal (`shouldRetry` returns false), so a single network round-trip surfaces `uapi.ErrAuthenticationFailed` instead of burning the 500ms+1s+2s retry budget. Authorization-header guard + User-Agent (`webox/<v>`) guard enforced on every request; missing-route returns 404 so a missing fixture surfaces at the specific call.

- **Sprint 22 TASK-22.1 + TASK-22.2 + TASK-22.3 — cPanel mutating client + full HostingProvider adapter + wizard integration (2026-05-26).** Second half of the cPanel adapter lands as a complete `providers.HostingProvider` implementation. **Mutating UAPI client** ([`providers/cpanel/uapi/mutating.go`](./providers/cpanel/uapi/mutating.go), [`providers/cpanel/uapi/mutating_https.go`](./providers/cpanel/uapi/mutating_https.go), [`providers/cpanel/uapi/mutating_ssh.go`](./providers/cpanel/uapi/mutating_ssh.go), [`providers/cpanel/uapi/mutating_composite.go`](./providers/cpanel/uapi/mutating_composite.go)) ships the typed `Mutator` interface with 16 methods covering domains (addon + subdomain create/delete), Passenger apps (create / edit / restart / delete), MySQL (db + user + privileges), and SSL (manual install + AutoSSL trigger + delete). Every method short-circuits behind the **env-var guard** `WEBOX_CPANEL_MUTATIONS=1` so a misconfigured caller cannot silently mutate a live panel; the transport itself enforces this in `guardAndValidate` BEFORE any I/O. HTTPS and SSH transports share a single argument-rendering surface (`argsFor*` free functions in `mutating.go`); the composite layer falls over on `ErrTransportUnavailable` exactly like the read-only `Composite`. **Idempotency error mapping** lives in `classifyMutationError`: documented cPanel "already exists" / "already in use" / "duplicate" / "is in use" wordings map to `ErrResourceExists`; "not exist" / "no such" / "not found" / "does not exist" / "no ssl certificate" / "is not installed" map to `ErrResourceNotFound`. The mapping is intentionally narrow — anything else surfaces verbatim. A retry-loop optimisation marks `ErrAPIResultFailure` as terminal in `transport.shouldRetry` so panel-level failures stop wasting wall-time on retries (test suite dropped from 18 s to 1.5 s). **cPanel adapter** ([`providers/cpanel/cpanel.go`](./providers/cpanel/cpanel.go), [`providers/cpanel/methods.go`](./providers/cpanel/methods.go), [`providers/cpanel/config.go`](./providers/cpanel/config.go), [`providers/cpanel/validators.go`](./providers/cpanel/validators.go)) implements every `HostingProvider` method on top of the typed seams (`uapi.Reader`, `uapi.Mutator`, `uapi.SSHRunner`) injected via `SetReader` / `SetMutator` / `SetSSHRunner`. Properties parsing applies typed defaults (`restart_method=passenger`, `node_selector=cloudlinux_selector`, `ssl_provider=autossl`, `domain_kind=addon`, app/deploy/log path templates rooted at `/home/{user}/nodejs/{app_root}`) and rejects out-of-range / shell-injected values with `ErrInvalidProviderConfig`. Validators (`ValidateDomain`, `ValidateNodeVersion`, `ValidateDBName`) registered via `providers.RegisterPlanValidators("cpanel", ...)` so the wizard resolves them through the same `PlanValidatorsFor` indirection it uses for smallhost. **CreateSubdomain** orchestrates `AddAddonDomain → CreatePassengerApp` (or `AddSubdomain → CreatePassengerApp` when `domain_kind=subdomain`); **SetupSSL** triggers AutoSSL and optionally settles for `autossl_poll_seconds`; **CreateDatabase** runs the three-step create_database → create_user → set_privileges sequence with best-effort rollback when the user / privileges step fails; **RestartNodeApp** maps `ErrResourceNotFound` to `providers.ErrAppNotFound`; **TailLog** tails `app.log` + `error.log` over SSH with clamped bounds (default 200, cap 10000) and stderr passthrough on non-zero exit; **Remove*** methods are idempotent (panel "not found" → nil); **CheckStatus** probes `DomainInfo.list_domains` and translates auth failure to `ErrCLINotFound`. Tests: 36 cases across [`providers/cpanel/methods_test.go`](./providers/cpanel/methods_test.go) (HostingProvider compliance, happy paths for every method, idempotency, rollback ordering, seam-missing detection, line clamping, latency clock seam) + [`providers/cpanel/config_test.go`](./providers/cpanel/config_test.go) (12 default + override + reject cases) + [`providers/cpanel/validators_test.go`](./providers/cpanel/validators_test.go) (24 cases: domain shape, node version, DB name, with explicit shell-injection rejections). **Wizard×cPanel integration** ([`wizard/cpanel_integration_test.go`](./wizard/cpanel_integration_test.go)) adds 4 tests proving the wizard's provider-agnostic orchestration works end-to-end against the cpanel adapter when the `cpanel-cloudlinux-selector` preset is in play: happy-path subdomain → SSL → DB with all three cleanup steps pushed onto the LIFO stack; DB-failure scenario unwinds SSL + subdomain (verifying the `DeleteSSL → DeletePassengerApp → DeleteDomain` rollback ordering); `PlanValidatorsFor("cpanel")` resolution; duplicate-domain pre-check via `ListSubdomains`.
- **Sprint 22 — Live cPanel smoke-test script ([`scripts/smoke-cpanel.sh`](./scripts/smoke-cpanel.sh), 2026-05-26).** Read-only operator script that exercises `webox doctor cpanel --json` against a real cPanel host using an API token sourced from the macOS Keychain (or Linux `secret-tool`). Never reads from disk, never writes the token to env vars that survive the shell, never touches a mutating endpoint (`WEBOX_CPANEL_MUTATIONS` is explicitly unset before every call). Optional `--capture` writes the redacted JSON report into `providers/cpanel/uapi/testdata/live/` (now gitignored) so the operator can review payloads BEFORE promoting any to a checked-in fixture. The script is the recipe Sprint 21 retro flagged as the missing live-account workflow; it intentionally uses API tokens (not the account password) so credentials can be rotated independently of the panel login.
...
- **Sprint 22 — Mutating cPanel UAPI ops gated behind explicit env-var opt-in (2026-05-26).** The new `Mutator` surface in `providers/cpanel/uapi` returns `ErrMutationsDisabled` from every method when `WEBOX_CPANEL_MUTATIONS=1` is not set. The check fires at method entry (before any I/O) so a misconfigured caller cannot silently mutate a live panel; the value is captured at call time (no caching) so a running process cannot inherit the opt-in from a previous shell. Defence-in-depth: the adapter layer (`providers/cpanel`) also gates its own property toggles, and the smoke-test script `scripts/smoke-cpanel.sh` explicitly `unset`s the variable before each call. See [`providers/cpanel/uapi/mutating.go`](./providers/cpanel/uapi/mutating.go) docstring on `EnvMutationsAllowed`.
- **Sprint 22 — cPanel argument validation rejects shell metacharacters + control chars at the transport boundary (2026-05-26).** Every typed argument struct (`CreateAddonDomainArgs`, `CreateSubdomainArgs`, `CreatePassengerAppArgs`, `EditPassengerAppArgs`, `MysqlPrivilegesArgs`, `InstallSSLArgs`) implements `Validate()` and short-circuits with `ErrInvalidArgs` when any field is empty / contains an ASCII control character (newline, NUL, carriage return). The check is defence-in-depth: the adapter's `ValidateDomain` / `ValidateNodeVersion` / `ValidateDBName` regex validators already reject these at the public boundary, but the transport-level check catches a hand-built `args` struct that bypassed the adapter. Test coverage: 24 cases in [`providers/cpanel/validators_test.go`](./providers/cpanel/validators_test.go) with explicit shell-injection / newline / uppercase-rejection rows.
- **Dev toolchain bumped to go1.26.4 to clear stdlib advisories (2026-06-09).** `make vulncheck` (a required input to the CI summary gate) started failing on any fresh run because [`tools/go.mod`](./tools/go.mod) pinned the `go1.26.3` toolchain that `GOTOOLCHAIN` derives for lint/format/vuln, and govulncheck flags it for **GO-2026-5037** (inefficient candidate-hostname parsing in `crypto/x509`, reached via `httpcheck.ProbeTLS` → `tls.DialWithDialer`) and **GO-2026-5039** (un-escaped inputs in `net/textproto` error strings, reached via `secrets.ReadMasterPassword` → `io.ReadAll`). Both are fixed in go1.26.4. Bumping the tools-module `go` directive moves the vuln/lint toolchain to 1.26.4; `make vulncheck` now reports `No vulnerabilities found`. Note: the shipped binary still builds against the `go.mod` `go 1.25.0` floor (CI uses `go-version-file: go.mod`); raising that build floor is a separate maintainer decision flagged for follow-up.
- **Redactor scrubs `curl --user <user>:<key>` + corpus false-positives repaired (2026-06-09, post-Sprint-23 polish).** [`internal/log/redact.go`](./internal/log/redact.go) gains a rule for the `--user '<user>:<loginkey>'` shape the DirectAdmin SSH loopback fallback emits via `shellQuote` ([`providers/directadmin/api/ssh.go`](./providers/directadmin/api/ssh.go)); the username is preserved for post-incident triage while the credential after the colon is replaced, covering both the bare and shell-quoted forms. Defence-in-depth — the command is not logged today, but the planned `--debug-trace` sink must never capture it. Separately, three corpus cases (`secret=…`, `Authorization: Bearer …`, `Authorization: Basic …`) called `base64Random(N)` **twice** — once for the input line, once for the expected-removed list — so the "secret scrubbed" assertion compared two unrelated random strings and passed regardless of whether redaction actually fired. Each opaque value is now generated once and reused, making the assertions real. Files: [`internal/log/redact_corpus_test.go`](./internal/log/redact_corpus_test.go).
