...
func TestRun_DoctorPreset_AcceptsTokensAndDispatches(t *testing.T) {
	...
	for _, tt := range cases {
		...
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			var stdout, stderr bytes.Buffer
			got := runWithFullDeps(tt.args, &stdout, &stderr, runDoctor, runDoctorGitHub, stub, brokenCpanelDispatcher, brokenDirectadminDispatcher, brokenTUI)
			if got != exitOK {
				t.Fatalf("exit = %d (stderr=%q)", got, stderr.String())
			}
...
func TestRun_DoctorPreset_RejectsProbeWithoutID(t *testing.T) {
	...
	got := runWithFullDeps(
		...
		&stdout, &stderr,
		runDoctor, runDoctorGitHub, stub, brokenCpanelDispatcher, brokenDirectadminDispatcher, brokenTUI,
	)
	if got != exitMisuse {
		t.Fatalf("exit = %d, want exitMisuse=%d", got, exitMisuse)
	}
...
// brokenDirectadminDispatcher mirrors [brokenCpanelDispatcher]
// for the doctor-directadmin route.
func brokenDirectadminDispatcher(_ directadminOpts, _, stderr io.Writer) int {
	_, _ = stderr.Write([]byte("brokenDirectadminDispatcher should not be reached\n"))
	return exitGeneric
}
